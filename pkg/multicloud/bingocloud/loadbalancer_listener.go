...
func (self *SElbListener) Refresh() error {
	listener, err := self.lb.GetILoadBalancerListenerById(self.GetId())
	if err != nil {
		return err
	}
...
func (self *SElbListener) GetScheduler() string {
	group, err := self.getTargetGroup()
	if err != nil {
		return ""
	}
	return group.GetScheduler()
}
...
func (self *SElbListener) getTargetGroup() (*SElbBackendGroup, error) {
	if self.group != nil {
		return self.group, nil
	}

	lbbg, err := self.lb.region.GetElbBackendGroup(self.DefaultActions[0].TargetGroupId)
	if err != nil {
...
func (self *SElbListener) GetBackendGroupId() string {
	return self.DefaultActions[0].TargetGroupId
}
...
func (self *SElbListener) ChangeCertificate(ctx context.Context, opts *cloudprovider.ListenerCertificateOptions) error {
	params := map[string]string{
		"ListenerArn":                          self.ListenerId,
		"Certificates.member.1.CertificateArn": opts.CertificateId,
	}
	_, _ = self.lb.region.invoke("ModifyListener", params)
	return nil
}
...
func (self *SRegion) GetElbListeners(elbId string) ([]SElbListener, error) {
	...
	resp, err := self.invoke("DescribeListeners", params)
	if err != nil {
		return nil, err
	}
	...
	err = resp.Unmarshal(&ret, "DescribeListenersResult")
	return ret.Listeners, err
}
...
func (self *SRegion) CreateElbListener(lbId string, opts *cloudprovider.SLoadbalancerListenerCreateOptions) (*SElbListener, error) {
	...
	resp, err := self.invoke("CreateListener", params)
	if err != nil {
		return nil, errors.Wrapf(err, "CreateListener")
	}

	var ret []*SElbListener
	err = resp.Unmarshal(&ret, "CreateListenerResult", "Listeners")
	if err != nil {
		return nil, errors.Wrapf(err, "CreateListener")
	}
...
func (self *SRegion) DeleteElbListener(id string) error {
	_, err := self.invoke("DeleteListener", map[string]string{"ListenerArn": id})
	return err
}
